...
const User = ({ user }: UserProps) => {
  const latestMessage = user.messages?.slice(-1)[0]
  ...
  return (
    <div className='grid grid-cols-[40px_1fr_50px] grid-rows-[40px] gap-3 py-3 px-4 border-b-[1px] hover:cursor-pointer hover:bg-orange-500'>
      <div className=''>
        <Avatar src={user.opponentUser.image} />
      </div>
      <div>
        <h3>{user.opponentUser.name}</h3>
        {latestMessage && (
      ...
      <div className='flex justify-end text-xs text-gray-500 whitespace-nowrap'>
        {latestMessage && <p>{fromNow(latestMessage.updateAt)}</p>}
      </div>
    </div>
  )
}
